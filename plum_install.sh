...
if [ -x "$(command -v dnf)" ]; then
    ...
    sudo pacman -S tcl boost boost-libs git cmake make gcc gcc-libs python which python-pylint clang docker || (echo "=> Error: dependency install went wrong"; exit 1) && goto end_deps
else
